		...
		forLoop: for (int step = 0, index = 0, first = 0, count = SITEMAP_MAX_SIZE;;) {
			...
			case 0:
				...
				if (articles.size() < count) {
					...
					generateCount += generate(sitemapTemplateConfig.getRealTemplatePath(), sitemapTemplateConfig.getRealStaticPath(model), model);
					articleDao.flush();
					articleDao.clear();
				...
				break;
			case 1:
				List<Goods> goodsList = goodsDao.findList(first, count, null, null);
